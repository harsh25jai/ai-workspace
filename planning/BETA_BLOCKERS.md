...
|---|---|---|---|---|
| C1 | Human `init` disabled | Agent A | `ai-workspace init` completes without agent env; creates `.ai/` structure | `AI_AGENT=false ai-workspace init` exits 0 |
| C2 | No `config.json` on init | Agent A | `config.json` always created with safe defaults | `test -f .ai/config.json` after init |
...
| C4 | Local provider corrupts docs | Agent A | Stub never overwrites `.ai/*.md`; template mode is default | `generate` output ≠ "Simulated Local Output" |
| C5 | `sync` broken | Agent A | Missing context triggers analyze; skill actions execute | `sync` with no context runs analyze |
| C6 | `explain` missing | Agent A | `explain <file>` implemented | `ai-workspace explain src/cli/index.ts` exits 0 |
