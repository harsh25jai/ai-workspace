# Project Analysis: `ctxstack`

## Overview
`ctxstack` is a Node.js-based Command Line Interface (CLI) tool designed to bridge the gap between source code repositories and Artificial Intelligence (AI) assistants. It scans a project to generate a structured, machine-readable "AI-native context layer." This helps AI agents understand the repository's architecture, dependencies, patterns, and rules without needing to ingest all source code files individually.

All generated context is securely stored within a local `.ctxstack/` directory inside the user's project root.
...
*   **`src/context/` & `src/workspace/`**: Handling state management, hashing, and configuration logic within the generated `.ctxstack/` folder.

## Core Workflows
Users interact with the CLI through several key commands:
1.  **`ctxstack init`**: Initializes the `.ctxstack/` environment, creates `.ctxstack/repo-map.json`, and sets up provider configurations (API keys) and placeholder context files.
2.  **`ctxstack analyze`**: Scans the codebase to extract dependency graphs, file architecture, and produces `.ctxstack/context/repo-context.json`.
3.  **`ctxstack generate`**: Utilizes the analysis data to compose readable documents: `project.md`, `architecture.md`, and `rules.md`.
4.  **`ctxstack status`**: Verifies if the `.ctxstack/` documentation is in-sync with the current state of the codebase.
5.  **`ctxstack sync`**: Incrementally updates the workspace context after file additions or changes.
6.  **`ctxstack regenerate`**: Forces a fresh build of all AI documentation.
7.  **`ctxstack config`**: Interactively updates LLM providers or API configurations.
...
*   **Integrity**: Modifying anything within `.ctxstack/` necessitates an updated state hash mechanism commonly managed by `state.json`.
