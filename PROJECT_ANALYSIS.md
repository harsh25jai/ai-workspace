...
1.  **`ctxstack init`**: Initializes the `.ai/` environment, creates `.ai/repo-map.json`, and sets up provider configurations (API keys) and placeholder context files.
2.  **`ctxstack analyze`**: Scans the codebase to extract dependency graphs, file architecture, and produces `.ai/context/repo-context.json`.
3.  **`ctxstack generate`**: Utilizes the analysis data to compose readable documents: `project.md`, `architecture.md`, and `rules.md`.
4.  **`ctxstack status`**: Verifies if the `.ai/` documentation is in-sync with the current state of the codebase.
